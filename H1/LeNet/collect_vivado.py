#!/usr/bin/env python3
"""Append Vivado OOC post-synthesis utilization and timing to the H1 table."""
import csv
import json
import re
...
def parse_rf(rf):
    ...
    values.update({
        ...
        "vivado_clock_period_ns": float(clock_match.group(1)),
        "vivado_clock_frequency_mhz": float(clock_match.group(2)),
        "vivado_wns_ns": float(timing_match.group(1)),
        "vivado_tns_ns": float(timing_match.group(2)),
        "vivado_post_synth_critical_path_ns": round(float(clock_match.group(1)) - float(timing_match.group(1)), 3),
        "vivado_post_synth_fmax_mhz_theoretical": round(1000 / (float(clock_match.group(1)) - float(timing_match.group(1))), 3),
...
if [int(row["global_rf_requested"]) for row in rows] != list(RFS):
    raise RuntimeError("Unexpected RF rows in resultados_hls.csv")

for row in rows:
    rf = int(row["global_rf_requested"])
    row.update(parse_rf(rf))
    clock_ns = float(row["clock_ns"])
    rtl_latency_cycles = float(row["rtl_latency_max_cycles"])
    rtl_interval_cycles = float(row["rtl_interval_max_cycles"])
    row["rtl_latency_us_at_target_clock"] = round(rtl_latency_cycles * clock_ns / 1000, 3)
    row["rtl_interval_us_at_target_clock"] = round(rtl_interval_cycles * clock_ns / 1000, 3)
    row["rtl_theoretical_images_per_second"] = round(1e9 / (rtl_interval_cycles * clock_ns), 3)
    comparisons = (

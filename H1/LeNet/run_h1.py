#!/usr/bin/env python3
"""Build the three H1 LeNet IPs using the original Resource flow."""
import json
...
for rf in (32, 64, 128):
    ...
    staging = Path(tempfile.mkdtemp(prefix=f"h1_lenet_rf{rf}_"))
    command = [sys.executable, str(ROOT / "generate_lenet_h1.py"),
               "--reuse-factor", str(rf), "--output-dir", str(staging)]

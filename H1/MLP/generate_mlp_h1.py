...
Target configuration:
  ...
  * ReuseFactor = 1, 4, 8, 16, or 32 (the only experimental variable)
  * Strategy = Latency
  * IOType = io_parallel
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--reuse-factor", type=int, choices=(1, 4, 8, 16, 32), required=True)
    parser.add_argument(
        "--skip-build",
        action="store_true",
...
def main() -> int:
    ...
    for layer in hls_model.get_layers():
        row = {
            ...
            "strategy": layer.get_attr("strategy"),
        }
        effective_layers.append(row)
        if row["class"] in {"Dense", "VitisDense"}:
            if row["reuse_factor"] != reuse_factor or row["strategy"] != "latency":
                raise RuntimeError(f"Unexpected effective Dense configuration: {row}")
    write_json(output_dir / "effective_layers.json", effective_layers)
    hls_model.compile()

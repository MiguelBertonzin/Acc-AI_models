...
def main():
    ...
    print(
        f"[PRECHECK] OK: {validation['correct']}/30 corretas, "
        f"{validation['reference_agreement']}/30 concordância INT8, "
        f"{validation['divergences']} divergências",
        flush=True,
    )
    ...
    print(f"[WARMUP] concluído: {a.warmup} inferências (fora da medição)", flush=True)
    ...
    print(
        f"[BASELINE] concluído: {a.baseline_seconds:.1f} s, "
        f"P_idle={idle_power_w:.6f} W",
        flush=True,
    )
    ...
    print(
        f"[BENCHMARK] iniciando {n} inferências medidas; "
        "sem prints/progress durante a janela para não perturbar os resultados",
        flush=True,
    )
    ...
    print(f"[BENCHMARK] concluído: {n} inferências medidas", flush=True)

...
|---|---:|---:|---:|
| Inference-only — média | 0.185747 ms | 0.183677 ms | -1.11% |
| Inference-only — mediana | 0.160364 ms | 0.161358 ms | +0.62% |
| Inference-only — desvio-padrão | 0.065265 ms | 0.057457 ms | -11.96% |
| Inference-only — p95 | 0.318122 ms | 0.296632 ms | -6.76% |
| Inference-only — p99 | 0.448947 ms | 0.411360 ms | -8.37% |
| End-to-end — média efetiva | 0.280414 ms | 0.271231 ms | -3.27% |
| End-to-end — vazão efetiva | 3566.15 inf/s | 3686.89 inf/s | +3.39% |
| Potência média da placa | 28.520 W | 29.471 W | +3.34% |
| Energia total por inferência | 8.158457 mJ | 8.193096 mJ | +0.42% |
| Utilização média da GPU | 10.49 % | 11.82 % | +12.71% |
| Temperatura média | 45.42 °C | 61.38 °C | +35.14% |
...
O teste t de Welch foi aplicado às médias das passagens completas de 30 inferências (1.000 passagens na referência e 3.333 na campanha nova), sem assumir variâncias iguais.

| Janela | Diferença 100k − 30k | IC95% da diferença | p bilateral | Diferença padronizada |
|---|---:|---:|---:|---:|
| Inference-only | −2,070 µs | [−5,626; 1,486] µs | 0,254 | −0,043 |
| End-to-end efetiva | −9,183 µs | [−13,077; −5,290] µs | 4,03×10⁻⁶ | −0,170 |

Na janela inference-only, o intervalo inclui zero: a redução observada de 1,11% não é estatisticamente distinguível da variação entre passagens. No end-to-end, o intervalo não inclui zero, mas a diferença padronizada é pequena. Como as campanhas ocorreram em sessões sequenciais e têm autocorrelação temporal, o teste é exploratório e não demonstra causalidade.
...
|---|---:|
| Média | 0.183677 ms |
| IC95% da média | [0.182193, 0.185161] ms |
| Mediana | 0.161358 ms |
| Desvio-padrão | 0.057457 ms |
| CV | 31.28% |
| p90 / p95 / p99 | 0.247629 / 0.296632 / 0.411360 ms |
| Mínimo / máximo | 0.135422 / 1.795105 ms |
...
| Métrica | Resultado |
|---|---:|
| Média individual | 0.270147 ms |
| IC95% da média | [0.268392, 0.271901] ms |
| Mediana | 0.256691 ms |
| Desvio-padrão | 0.073043 ms |
| p90 / p95 / p99 | 0.337682 / 0.407275 / 0.555160 ms |
| Mínimo / máximo | 0.177934 / 2.222032 ms |
| Média efetiva pelas passagens | 0.271231 ms |
| Vazão efetiva | 3686.89 inferências/s |
...
| Métrica | Resultado |
|---|---:|
| Amostras durante benchmark | 274 |
| Janela energética | 27.801 s |
| Potência média / p95 / máxima | 29.471 / 29.710 / 31.340 W |
| Potência ociosa | 29.139 W |
| Energia total | 819.310 J |
| Energia total por inferência | 8.193096 mJ |
| Energia dinâmica por inferência | 0.092125 mJ |
| Utilização GPU média / p95 / máxima | 11.82% / 14.00% / 15.00% |
| Memória usada média / máxima | 875.0 / 875.0 MiB |
| Clock SM médio / máximo | 1755.0 / 1755.0 MHz |
| Temperatura média / máxima | 61.4 / 63.0 °C |
| Ventoinha média / máxima | 5.4% / 43.0% |
| P-state | P2: 274 |

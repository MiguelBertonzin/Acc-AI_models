#!/usr/bin/env python3
"""Compila o H5 quantizado para o alvo DPUCZDX8G/ZCU104."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--arch",
        type=Path,
        default=ROOT / "config/arch_zcu104_vai3.5.json",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "artifacts/compiled/resnet8_no_softmax_zcu104_vai3_5",
    )
    parser.add_argument("--net-name", default="resnet8_no_softmax")
...
def main() -> None:
    ...
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise FileExistsError(
            f"{args.output_dir} não está vazio; use outro --output-dir para preservar a execução"
        )
    args.output_dir.mkdir(parents=True, exist_ok=True)
    log_path = ROOT / "logs/compiler/compile_resnet8_no_softmax.log"
    ...
    if report["status"] != "passed":
        raise RuntimeError(
            f"Compilação falhou; consulte {log_path} e {report_path}"
        )
